...
6. **`pipeline.py`** — `extract_titles(titles, client, ...)` orchestrates the above. The `LLMClient` is **injected explicitly**; there is no init-first global in the new API. `send_batches` chunks into `batch_size` (`utils.chunk_list`, one LLM call per chunk) and **latches structured output off for the rest of the run** in two cases: (a) the server rejects `response_format` (exception — that batch is immediately re-sent plain), and (b) a batch returns 200 but with **fewer items than it was given** (truncation — the missing items are left to the partial retry, not re-sent here).
   - **Why (b) exists**: with `response_format` set, gemma-4-e2b on LM Studio emits only the **first array element** and stops — measured `finish_reason=stop` / `completion_tokens=37` / **`reasoning_tokens=0`**. The identical prompt without `response_format` returns **every** item (`reasoning_tokens=555`). Constrained decoding leaves no room for thinking tokens, so this is **deterministic while the schema is attached**, not flaky. Symptom before 0.4.0: `extract_titles(...)` with n≥2 returned `title=""` for everything after the first item.
   - **Partial retry** (`retry_invalid`, default 1) is **independent of `bypass_check`** as of 0.4.0. `bypass_check` decides only whether a still-failing run raises `ValueError` at the end; whether recovery is attempted is `retry_invalid`'s job (`retry_invalid=0` disables it). So `bypass_check=True` can now cost extra round trips — documented in the `extract_titles` docstring.
   - Retries always go out with **`use_schema=False`**, since re-asking under the same constraint just replays the same truncation. Invalid items are split by failure mode, one `send_batches` call each: **missing** (no output for that index — placeholder with an empty title) goes at `temperature=0.0` on the first attempt (dropping the schema is the actual fix) and at `_RETRY_TEMPERATURE` from the second; **mismatch** (a title came back but failed `is_title_match`) goes at `_RETRY_TEMPERATURE` (0.4) right away, so a deterministic failure isn't replayed verbatim.
