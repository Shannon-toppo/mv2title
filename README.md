...
3. `.env.example` を `.env` にコピーし、`BASE_URL`（必須）・`API_KEY`・`SYSTEM_PROMPT`・`MODEL` を設定する。
...
1. **開発基盤の整備** — ruff の導入（ファイル間でタブ/スペースが混在しているインデントの統一を含む）、GitHub Actions による CI（pytest + ruff）、LICENSE ファイルの追加、`pyproject.toml` のメタデータ（description 等）の整備。
