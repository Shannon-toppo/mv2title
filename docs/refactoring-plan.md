...
## フェーズ 5: 品質ゲートとドキュメント ✅ 実施済み (2026-07-02)

| タスク | 委譲 | 状態 |
|---|---|---|
| 型チェッカ(pyright, standard モード)を dev 依存 + CI に追加し、型エラーを解消 | 【O】 | ✅ 修正は 8 件のみ(`channels` の `Sequence` 化など) |
| ruff ルール拡充(`SIM`, `C4`, `RUF`, `PT`, `ARG`)と指摘解消 | 【S】 | ✅ RUF001-003(曖昧 Unicode)は全角文字処理が本質のため ignore。ARG/C408 は tests のみ ignore |
| README 刷新(新 API クイックスタート、`.env` テンプレート例) | 【S】ドラフト → 【F】レビュー | ✅ フェーズ 3〜4 で段階的に更新済み(`.env` 例は準備節に記載あり) |
...
- **残る作業** (2026-07-02 全フェーズ完了時点): パースフォールバック(`ast.literal_eval` / カンマ分割)の削除のみ。実運用で warning の発動が無いことを確認できたら削除する(README の Roadmap にも記載)。
